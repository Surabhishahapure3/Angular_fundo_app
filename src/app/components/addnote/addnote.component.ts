...
export class AddnoteComponent implements OnInit {
  ...
  handleNotesIconsClick(action: string, color: string= '#ffffff'){
    if(action==='color'){
      ...
      this.noteService.changeColorById('router', this.noteDetails._id, color).subscribe({
        next:(res: any)=>{
  ...
  toggleTakeNote(addNote:boolean=false){
    ...
    //change here 
    if(addNote && !this.data){
      this.noteService.createNoteApiCall("router",{Title: this.title, Description:this.description}).subscribe({
        next:(data:any)=>{
